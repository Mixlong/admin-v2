              ...
              <el-row :gutter="20">
                ...
                <el-col :span="12">
                  <el-form-item label="生产地点" prop="address">
                    ...
                    >
                      <el-option
                        v-for="(item, index) in filteredProductAddressList"
                        :key="index"
            ...
            <el-row :gutter="20">
              <el-col :span="24">
                <el-form-item label="采购订单图" prop="purchaseOrderImg">
                  <ImageUpload
                    v-model="formData.purchaseOrderImg"
                    :accept="'image/*'"
                    :max="1"
                    :sortable="false"
                    :isDisabled="formData.auditStatus >= 1"
                    class="purchase-order-img-upload"
                  />
                </el-form-item>
              </el-col>
            </el-row>
    ...
    >
      <div v-if="viewData" class="detail-container">
        <el-descriptions :column="3" border>
          ...
          <el-descriptions-item label="生产流程">{{ viewData.productionProcess || '--' }}</el-descriptions-item>
          <el-descriptions-item label="生产地点">{{ viewData.address || '--' }}</el-descriptions-item>
          <el-descriptions-item label="采购单号">{{ viewData.purchaseOrderCode || '--' }}</el-descriptions-item>
          <el-descriptions-item label="请购单号">{{ viewData.purchaseRequestCode || '--' }}</el-descriptions-item>
          <el-descriptions-item label="出货日期">
            {{ viewData.sellDate ? parseTime(viewData.sellDate, '{y}-{m}-{d}') : '--' }}
          </el-descriptions-item>
          ...
          <el-descriptions-item label="初审者">{{ viewData.firstAuditor || '--' }}</el-descriptions-item>
          <el-descriptions-item label="初审时间"   >
            {{ viewData.firstAuditTime ? parseTime(viewData.firstAuditTime, '{y}-{m}-{d} {h}:{i}:{s}') : '--' }}
          </el-descriptions-item>
          <el-descriptions-item label="发布时间" >
            {{ viewData.publishTime ? parseTime(viewData.publishTime, '{y}-{m}-{d} {h}:{i}:{s}') : '--' }}
          </el-descriptions-item>
          <el-descriptions-item label="终审者" >{{ viewData.finalAuditor || '--' }}</el-descriptions-item>
          <el-descriptions-item label="终审时间"  >
            {{ viewData.finalAuditTime ? parseTime(viewData.finalAuditTime, '{y}-{m}-{d} {h}:{i}:{s}') : '--' }}
          </el-descriptions-item>
          <el-descriptions-item label="硬件版本号">{{ viewData.hwVersion || '--' }}</el-descriptions-item>
        ...
        <div class="material-section">
          ...
          >
            ...
            <el-table-column prop="fileType" label="文件类型" align="center" width="120">
              <template slot-scope="scope">
                <el-tag size="small" type="info">{{ scope.row.fileType || '--' }}</el-tag>
              </template>
            </el-table-column>
            ...
            <el-table-column label="操作" align="center" width="120">
              <template slot-scope="scope">
                <el-button
                  ...
                  @click="handleDownloadFile(scope.row.fileUrl, scope.row.fileName)"
                >
                  下载
                </el-button>
...
export default {
  ...
  data() {
    return {
      ...
      formRules: {
        schedulingNo: [
          { required: true, message: '请扫码任务令或输入排产单号', trigger: 'blur' }
        ],
        productionProcess: [
  ...
  computed: {
    ...
    filteredProductAddressList() {
      return this.productAddressList.filter(item => item.dictLabel !== '迪太工厂')
    }
  },
  mounted() {
  ...
  methods: {
    ...
    fetchData() {
      ...
      getOutsourcingProductionList(params).then(response => {
    ...
    async getComputerNameList(name) {
      if (name) {
        ...
        if (this.searchForm.categoryId) {
          const category = this.categoryList.find(item => item.id === this.searchForm.categoryId)
          this.modelOptions = category?.computerList || []
        } else {
          this.modelOptions = []
        }
    ...
    async getFormComputerNameList(name) {
      if (name) {
        ...
        if (this.formData.categoryId) {
          const category = this.categoryList.find(item => item.id === this.formData.categoryId)
          this.formModelOptions = category?.computerList || []
        } else {
          this.formModelOptions = []
        }
    ...
    handleSubmit() {
      this.$refs.productionForm.validate(valid => {
        if (valid) {
          ...
          const apiMethod = this.isEdit ? updateOutsourcingProduction : addOutsourcingProduction
