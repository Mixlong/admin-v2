<template>
  <el-dialog title="批量导入打板数据" :visible.sync="dialogVisible" width="900px" :close-on-click-modal="false"
...
<script>
import { importOutsourcingProduction } from '@/api/outsourcing/production'
  ...
  methods: {
    ...
    async downloadTemplate() {
      try {
        ...
        link.href = 'http://file.riding-evolved.com/model/%E5%A4%96%E5%8D%8F%E7%94%9F%E4%BA%A7%E5%AF%BC%E5%85%A5%E6%A8%A1%E6%9D%BF.xlsx'
        link.download = '外协生产导入模板.xlsx'
    ...
    async handleImport() {
      ...
      try {
        ...
        const response = await importOutsourcingProduction(formData)
        ...
        if (response.code === 200) {
          ...
          if (response.data?.errors && Array.isArray(response.data.errors)) {
            errorInfo = response.data.errors.map(error => ({
              row: error.row || '-',
              error: error.message || error.msg || '未知错误'
            }))
          }

          // 如果没有详细错误信息，但有失败数量，显示通用提示
          if (failedCount > 0 && errorInfo.length === 0) {
            errorInfo = [{
              row: '-',
              error: `有 ${failedCount} 条数据导入失败，请检查数据格式`
            }]
          }
