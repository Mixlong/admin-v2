<template>
	<!--   -->
	<el-dialog :title="title" :visible.sync="dialogVisible" width="710px" append-to-body top="13vh"
		:close-on-click-modal="false">
		<el-form ref="form" :model="form" :rules="rules" label-width="95px" @submit.native.prevent
			class="form-data-inline" inline>
			<el-form-item label="装备类型:" prop="name">
				<select-loadMore v-model="form.name" style="width: 100%" :data="equipmentData.data"
					:page="equipmentData.page" :hasMore="equipmentData.more" dictLabel="name" :moreParams="true"
					:disabled="!!form.id" :request="getEquipmentList" :clearFn="clearCode" @getChange="getEqupmentInfo"
					placeholder="请选择装备类型" />
			</el-form-item>
			...
			<el-form-item label="系列名称:" prop="type">
				<el-select v-model="form.type" placeholder="请选择系列名称" style="width: 100%" clearable @change="form.module = ''">
					<el-option v-for="(item, index) in dictOptions" :key="index" :label="item.dictLabel"
						:value="item.dictValue">
					</el-option>
				</el-select>
			</el-form-item>
			<el-form-item label="模块名称:" prop="module">
				<el-select v-model="form.module" placeholder="请选择模块名称" style="width: 100%" clearable>
					<el-option v-for="(item, index) in moduleOptions" :key="index" :label="item.dictLabel"
						:value="item.dictValue">
					</el-option>
				</el-select>
			</el-form-item>
			<el-form-item label="覆盖条件:" prop="updateCondition">
				<el-select v-model="form.updateCondition" placeholder="请选择覆盖条件" style="width: 100%"
					@change="changeTargetIdType">
					<el-option v-for="item in conditionOptions" :key="item.value" :label="item.dictValue"
						:value="+item.dictLabel">
					</el-option>
				</el-select>
			</el-form-item>
			<el-form-item label="版本号:" prop="versionName">
				<el-input v-model="form.versionName" placeholder="请输入版本号" clearable>
				</el-input>
			</el-form-item>
			<el-form-item label="版本序号:" prop="versionCode">
				<el-input v-model="form.versionCode" placeholder="请输入版本序号" clearable>
				</el-input>
			</el-form-item>
			<el-form-item label="版本描叙:" prop="explains" class="form-textarea">
				<el-input type="textarea" :rows="4" v-model="form.explains" placeholder="请输入版本描叙">
				</el-input>
			</el-form-item>
