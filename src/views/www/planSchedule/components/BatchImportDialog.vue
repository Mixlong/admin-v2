<template>
  <el-dialog title="批量导入排产数据" :visible.sync="dialogVisible" width="900px" :close-on-click-modal="false"
...
<script>
import { schedulingImport } from '@/api/www/planSchedule'
  ...
  methods: {
    ...
    async downloadTemplate() {
      try {
        ...
        link.href = 'https://bikewise.oss-cn-shenzhen.aliyuncs.com/model/%E4%BB%BB%E5%8A%A1%E4%BB%A4%E5%AF%BC%E5%85%A5.xlsx'
        link.download = '任务令导入模板.xlsx'
    ...
    async handleImport() {
      ...
      try {
        ...
        const response = await schedulingImport(formData)
