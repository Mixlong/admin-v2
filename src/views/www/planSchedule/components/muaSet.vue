      ...
      if (data?.configList?.length) {
        ...
        if (jsonStr) {
          try {
            ...
            if (configObj) {
              const { upgradeProject, supplyVoltage, communicationType } = configObj
              this.form.configType = configType
              this.form.upgradeProject = upgradeProject || []
              this.form.supplyVoltage = supplyVoltage || '0'
              this.form.communicationType = communicationType || '0'
            }
          } catch (e) {
            console.error('解析MUA配置JSON失败', e)
          }
        }
      }
    },
