<template>
  <div class="">
    <el-dialog
      class="fixed_bottom_dialog plan-schedule-update"
      :title="title"
    ...
    >
      ...
      >
        <template v-if="!isExcelFile">
            ...
            <el-col :span="4">
              <el-form-item label="生产流程:" prop="process">
                <el-select
                  ...
                  style="width: 100%"
                  :disabled="form.id !== '' && (form.process.includes('组装') || form.process.includes('包装'))"
                >
                  <el-option
          ...
          <template v-if="form.id && editOrderTables.length > 0">
            <div
              v-for="(table,index) in editOrderTables"
              :key="table.key"
              :label="`客户订单信息(${table.label})`"
              required
            >
              <el-table
                :ref="'customerOrderRef_' + table.key"
                :data="table.data"
                border
                row-key="id"
                :show-header="index>0?false:true"
                :row-class-name="(args) => tableRowClassName(args, table.key)"
              >
                <el-table-column
                  v-if="!form.id"
                  type="selection"
                ...
                >
                  <template slot-scope="scope">
                    <el-date-picker
                      ...
                      style="width: 100%"
                      @change="
                        onDateRangeChange(scope.$index, scope.row, table.key)
                      "
                    />
                  </template>
                </el-table-column>
                ...
                <el-table-column label="排产数量" align="center" prop="num">
                  <template slot-scope="scope">
                    <div
                      class="schedule-num-container"
                      @click="
                        showDailySchedulePopover(
                          scope.$index,
                          scope.row,
                          $event
                        )
          ...
          <el-form-item v-else label="客户订单信息:" required>
            <el-table
              ...
              <el-table-column label="排产数量" align="center" prop="num">
                <template slot-scope="scope">
                  <div
                    class="schedule-num-container"
                    @click="
                      showDailySchedulePopover(scope.$index, scope.row, $event)
                    "
                  >
                    <el-input
                      v-model="scope.row.num"
    ...
    <!-- 每日排产浮层 - 移到对话框外部 -->
    <div
      v-if="dailySchedulePopover.visible"
      ref="dailySchedulePopover"
      class="daily-schedule-popover"
      :style="dailySchedulePopover.style"
    ...
    >
      <div class="popover-header">
        ...
        <div class="header-actions">
          ...
          <el-button
            size="mini"
            type="text"
            @click="closeDailySchedulePopover"
            class="close-btn"
            >×</el-button
          >
        </div>
        ...
        >
          <el-table-column label="日期" prop="date" width="120" align="center">
            <template slot-scope="scope">
              <span
                class="weekday-text"
                :class="{ weekend: scope.row.isWeekend }"
              >
          ...
          <el-table-column label="工作状态" width="120" align="center">
            <template slot-scope="scope">
              <el-select
                ...
                size="mini"
                @change="updateWorkStatus(scope.row)"
                style="width: 100%"
                popper-class="work-status-dropdown"
              >
          ...
          <el-table-column label="计划数量" width="120" align="center">
            <template slot-scope="scope">
              <el-input-number
                :controls="false"
                v-model="scope.row.plannedNum"
                :min="0"
                :precision="0"
                :disabled="scope.row.workStatus === 'rest'"
                size="mini"
                controls-position="right"
                @change="updateTotalScheduleNum"
                style="width: 100%"
              />
            </template>
          </el-table-column>
      ...
      <div class="popover-footer">
        <div class="total-info">
          ...
          <span class="working-days-info"
            >（{{ workingDaysCount }}个工作日）</span
          >
        </div>
        <div class="footer-actions">
          <el-button size="mini" @click="closeDailySchedulePopover"
            >取消</el-button
          >
          <el-button size="mini" @click="confirmDailySchedule" type="primary"
            >确定</el-button
          >
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    // 工作日数量
    workingDaysCount() {
      return this.dailyScheduleList.filter(
        (day) => day.workStatus === "working" || day.workStatus === "special"
      ).length;
    },
  },
  watch: {
  ...
  methods: {
    ...
    getDateRangeFromSchedule(schedule) {
      if (schedule?.detailList && schedule.detailList.length > 0) {
        const dates = schedule.detailList
          .map((detail) => detail.date)
          .sort((a, b) => a - b);
        const startDate = dates[0];
        const endDate = dates[dates.length - 1];
        return [this.formatTimestamp(startDate), this.formatTimestamp(endDate)];
      }
      if (schedule?.date && schedule?.endDate) {
        return [
          this.formatTimestamp(schedule.date),
          this.formatTimestamp(schedule.endDate),
        ];
      }
      return null;
    },
    buildOrderRowFromSchedule(schedule) {
      return {
        id: schedule.id,
        customerOrderNo: schedule.customerOrderNo || schedule.orderNo,
        batchNo: schedule.batchNo,
        batchNum: schedule.batchNum,
        num: schedule.num,
        dateRange: this.getDateRangeFromSchedule(schedule),
        categoryId: schedule.categoryId || "",
        computerId: schedule.computerId || "",
        customerName: schedule.customerName || "",
        categoryName: schedule.categoryName || "",
        computerName: schedule.computerName || "",
        orderId: schedule.orderId || "",
        bomCode: schedule.bomCode || "",
        chipVersion: schedule.chipVersion || "",
        orderQuantity: schedule.orderQuantity || 0,
        sellTime: schedule.sellTime || null,
        process: schedule.process || "",
      };
    },
    applyDetailListToRow(schedule, row) {
      if (!schedule?.detailList || schedule.detailList.length === 0) return;

      const dateRange = row.dateRange;
      let fullDailySchedules = [];
      if (dateRange && dateRange.length === 2) {
        fullDailySchedules = this.generateFullDateRange(
          dateRange[0],
          dateRange[1]
        );
      }
      ...
      if (fullDailySchedules.length > 0) {
        schedule.detailList.forEach((detail) => {
          ...
          const matchingDay = fullDailySchedules.find(
            (day) => day.date === detailDateStr
          );
          if (matchingDay) {
            matchingDay.plannedNum = detail.num;
            const dayOfWeek = new Date(detail.date).getDay();
            if (dayOfWeek === 0 && detail.num > 0) {
              matchingDay.workStatus = "working";
            }
          }
        });
      } else {
        fullDailySchedules = schedule.detailList.map((detail) => {
          const dayOfWeek = new Date(detail.date).getDay();
          return {
      ...
      const totalNum = fullDailySchedules.reduce(
        (sum, scheduleItem) => sum + scheduleItem.plannedNum,
        0
      );
      row.num = totalNum;
    ...
    applyPairedTables() {
      if (!this.primarySchedule) return;
      const list = this.pairedSchedules || [];
      const schedules = list.length > 0 ? list : [this.primarySchedule];
      const processOrder = { 组装: 1, 包装: 2 };
      const sortedSchedules = schedules
        .slice()
        .sort(
          (a, b) =>
            (processOrder[a.process] || 99) - (processOrder[b.process] || 99)
        );

      this.editOrderTables = sortedSchedules.map((schedule) => {
        const row = this.buildOrderRowFromSchedule(schedule);
        this.applyDetailListToRow(schedule, row);
        return {
          key: schedule.id,
          label: schedule.process || "计划",
          data: [row],
        };
      });
    },
    ...
    loadSingleOrderData(orderData) {
      ...
      this.loadPairedSchedules(orderData);
      this.primarySchedule = orderData;
      ...
      const orderRow = this.buildOrderRowFromSchedule(orderData);
      this.applyDetailListToRow(orderData, orderRow);
      ...
      this.editOrderTables = [
        {
          key: orderData.id,
          label: orderData.process || "计划",
          data: [orderRow],
        },
      ];
      this.applyPairedTables();
    ...
    loadPairedSchedules(orderData) {
      if (!orderData || !orderData.mainSchedulingNo) {
        this.pairedSchedules = [];
        return;
      }

      schedulingList({ mainSchedulingNo: orderData.mainSchedulingNo })
        .then((res) => {
          const list = res?.data?.list || [];
          this.pairedSchedules = list;
          this.applyPairedTables();
        })
        .catch(() => {
          this.pairedSchedules = [];
    ...
    loadMultipleOrdersData(scheduleData) {
      ...
      this.orderData = scheduleData.list.map((batchItem, index) => {
        ...
        if (batchItem.detailList && batchItem.detailList.length > 0) {
          ...
          const dates = batchItem.detailList
            .map((detail) => detail.date)
            .sort((a, b) => a - b);
          const startDate = dates[0];
          const endDate = dates[dates.length - 1];
        ...
        if (batchItem.detailList && batchItem.detailList.length > 0) {
          ...
          // 计算总排产数量
          const totalNum = fullDailySchedules.reduce(
            (sum, schedule) => sum + schedule.plannedNum,
            0
          );
    ...
    loadDropdownData(orderData) {
      ...
      if (orderData.salesOrderNo && this.orderCodeData.data.length === 0) {
        this.getCustomerOrderList({ keyword: orderData.salesOrderNo }).then(
          () => {
            // 确保当前值在选项中
            const exists = this.orderCodeData.data.some(
              (item) => item.salesOrderNo === orderData.salesOrderNo
            );
            if (!exists && orderData.salesOrderNo) {
              this.orderCodeData.data.unshift({
                salesOrderNo: orderData.salesOrderNo,
              });
            }
          }
      ...
      if (orderData.orderCode && this.prodPlatData.data.length === 0) {
        this.getProdPlantList({ keyword: orderData.orderCode }).then(() => {
          // 确保当前值在选项中
          const exists = this.prodPlatData.data.some(
            (item) => item.orderCode === orderData.orderCode
          );
    ...
    onAlertReason(params) {
      ...
      const cloneDate =
        this.cloneForm.date instanceof Date
          ? this.cloneForm.date.getTime()
          : this.cloneForm.date;
    ...
    onUpdateOrder(params) {
      ...
      console.log("🚀 ~ file: update.vue:735 ~ params:", payloads);
      Promise.all(payloads.map((payload) => schedulingEdit(payload)))
        .then((responses) => {
    ...
    buildEditPayloadsFromTables(baseData) {
      const tables =
        this.editOrderTables && this.editOrderTables.length > 0
          ? this.editOrderTables
          : [{ label: "计划", data: this.orderData }];
        ...
        .map((table) => {
          ...
          if (dailySchedules.length > 0) {
            detailList = dailySchedules
              .filter(
                (day) =>
                  (day.workStatus === "working" ||
                    day.workStatus === "special") &&
                  day.plannedNum > 0
              )
              .map((day) => ({
                batchNo: item.batchNo,
                categoryId: item.categoryId || baseData.categoryId || "",
                computerId: item.computerId || baseData.computerId || "",
          ...
          return {
            ...baseData,
            id: item.id,
            process: item.process || baseData.process,
            categoryId: item.categoryId || baseData.categoryId || "",
            computerId: item.computerId || baseData.computerId || "",
            batchNo: item.batchNo,
            ...
            orderId: item.orderId || baseData.orderId,
            date: this.getValidDate("", item.dateRange),
            endDate:
              item.dateRange && item.dateRange.length === 2
                ? new Date(item.dateRange[1]).getTime()
        ...
        if (valid) {
          ...
          if (data.id) {
            ...
            for (const table of tables) {
              ...
              if (!item.batchNum) {
                this.msgWarning(`${table.label}批次数量不能为空`);
                return;
              }

              if (!item.num || item.num <= 0) {
                this.msgWarning(`${table.label}排产数量必须大于0`);
                return;
              }
            }

            const updatePayloads = this.buildEditPayloadsFromTables(data);
            if (!updatePayloads.length) {
              this.msgWarning("未找到可编辑的排产数据");
              return;
            }
            ...
            if (!this.isExcelFile && this.onAlertReason(data)) {
              this.$prompt("请输入修改原因", "提示", {
                ...
                .then(({ value }) => {
                  const payloadsWithMsg = updatePayloads.map((payload) => ({
                    msg: value,
                    ...payload,
                  }));
                  this.onUpdateOrder(payloadsWithMsg);
                })
                .finally(() => {
                  this.isBtnLoading = false;
                });
            } else {
              this.onUpdateOrder(updatePayloads);
            }
          } else {
            ...
            if (
              this.checkDeliverOrderData(this.selOrderData, "num") ||
              this.selOrderData.some((item) => item.num <= 0)
            ) {
            ...
            data.list = this.selOrderData.map((item, index) => {
              ...
              return {
                ...
                num: item.num,
                date: item.dateRange
                  ? new Date(item.dateRange[0]).getTime()
                  : null,
            ...
            console.log(
              "📦 创建时第一个 detailList 包含批次数据:",
              data.list[0]?.detailList?.[0]
            );
    ...
    // 处理日期范围变化
    onDateRangeChange(rowIndex, rowData, tableKey) {
      ...
      this.$nextTick(() => {
        setTimeout(() => {
          ...
          // 方法1: 通过表格行索引和类名查找
          const tableRef = tableKey
            ? this.$refs[`customerOrderRef_${tableKey}`]
            : this.$refs.customerOrderRef;
          const tableInstance = Array.isArray(tableRef)
            ? tableRef[0]
            : tableRef;
          const tableBody = tableInstance?.$el?.querySelector(
            ".el-table__body tbody"
          );
          if (tableBody) {
            const rows = tableBody.querySelectorAll("tr");
            if (rows[rowIndex]) {
              triggerElement = rows[rowIndex].querySelector(
          ...
          if (!triggerElement) {
            const selector = tableKey
              ? `[data-table-key="${tableKey}"][data-row-index="${rowIndex}"] .schedule-num-container`
              : `[data-row-index="${rowIndex}"] .schedule-num-container`;
          ...
          if (!triggerElement) {
            console.warn(
              `⚠️ 未找到第 ${rowIndex} 行的任何元素，将使用屏幕中央定位`
            );
            triggerElement = null;
          }
    ...
    showDailySchedulePopover(rowIndex, rowData, event) {
      ...
      // 如果浮层已经显示且是同一行，则关闭
      if (
        this.dailySchedulePopover.visible &&
      ...
      console.log(
        "🔍 生成每日数据完成，dailyScheduleList长度:",
        this.dailyScheduleList.length
      );

      // 计算浮层位置
      const triggerElement =
        event.target.closest(".schedule-num-container") || event.currentTarget;
    ...
    autoDistributeSchedule() {
      ...
      const workingDays = this.dailyScheduleList.filter(
        (day) => day.workStatus === "working" || day.workStatus === "special"
      );
      ...
      this.$message.success(
        `已智能分配排产数量：总计${this.distributeAmount}，平均分配到${workingDays.length}个工作日`
      );
    },
    ...
    confirmDailySchedule() {
      // 检查是否有工作日设置了排产数量
      const workingDaysWithPlan = this.dailyScheduleList.filter(
        (day) =>
    ...
    saveDailyScheduleData() {
      ...
      if (this.dailySchedulePopover.currentRowData) {
        this.$set(
          ...
          "num",
          this.totalScheduleNum
        );
      }

      this.$message.success(
        `每日排产计划已保存，总排产数量：${this.totalScheduleNum}`
      );
      this.closeDailySchedulePopover();
    },
    ...
    tableRowClassName({ rowIndex }, tableKey) {
      ...
      this.$nextTick(() => {
        ...
        const rowElement = tableInstance?.$el?.querySelector(
          `.el-table__body tr:nth-child(${rowIndex + 1})`
        );
        if (rowElement) {
          rowElement.setAttribute("data-row-index", rowIndex);
          if (tableKey) {
            rowElement.setAttribute("data-table-key", tableKey);
          }
        }
      });
      return "";
