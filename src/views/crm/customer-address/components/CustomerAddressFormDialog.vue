  ...
  >
    ...
    >
      ...
      <el-form-item label="联系方式" prop="contactPhone">
        <el-input v-model="form.contactPhone" placeholder="请输入联系方式" maxlength="200" />
      </el-form-item>
...
<script>
import { addCustomerAddress, updateCustomerAddress, getCustomerAddress } from '@/api/crm/customerAddress'
...
export default {
  ...
  computed: {
    ...
    isCustomerDisabled() {
      ...
      return this.isEdit || (this.customerAddress && this.customerAddress.customerId && !this.customerAddress.id)
    }
  },
  watch: {
    visible(val) {
  ...
  methods: {
    ...
    resetForm() {
      ...
      const defaultCustomerId = this.customerAddress && this.customerAddress.customerId 
      ...
      if (!exists) {
        ...
        if (name) {
          ...
          try {
            ...
            const customer = filteredList.find(item => String(item.id) === String(id))
            if (customer) {
              this.customerData.data.unshift({ id: customer.id, name: customer.customerName })
            }
          } catch (error) {
    ...
    parseAddressText(text) {
      ...
      if (phoneMatch) {
        ...
        // 尝试提取姓名（手机号前面的部分）
        const beforePhone = trimmedText.substring(0, trimmedText.indexOf(contactPhone)).trim()
        
        // 清理分隔符
        const namePart = beforePhone.replace(/[\/\-\s]+$/, '').trim()
        if (namePart && namePart.length <= 20) { // 姓名一般不超过20个字符
          contactName = namePart
        }
        
        // 提取地址（手机号后面的部分）
        const afterPhone = trimmedText.substring(trimmedText.indexOf(contactPhone) + contactPhone.length).trim()
