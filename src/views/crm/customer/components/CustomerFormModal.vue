    ...
    >
      ...
      <fieldset class="form-fieldset contact-fieldset">
        ...
        <div v-else class="contact-table-wrapper">
          <el-table :data="contactList" border size="small" max-height="250">
            <el-table-column
              type="index"
              label="序号"
              width="55"
              ...
              <template slot-scope="{ row, $index }">
                ...
                <template v-else>
                  ...
                  <template v-else>
                    <div
                      v-if="
                        row.cardImage && getImageList(row.cardImage).length > 0
                      "
                      style="
                        display: flex;
                    ...
                    >
                      ...
                      <el-tag size="mini" type="info"
                        >{{ getImageList(row.cardImage).length }}张</el-tag
                      >
                    </div>
      ...
      <fieldset class="form-fieldset address-fieldset">
        ...
        <div v-else class="address-table-wrapper">
          <el-table :data="addressList" border size="small" max-height="250">
            <el-table-column
              type="index"
              label="序号"
              width="55"
...
export default {
      ...
      async handler(newCustomer) {
        if (newCustomer) {
          ...
          Object.assign(this.form, {
            ...
            // 业务信息字段
            paymentTerm: this.resolvePaymentTermDictCode(
              newCustomer.paymentTerm
            ),
          ...
          this.form.paymentTerm = this.resolvePaymentTermDictCode(
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      if (!this.isEdit) {
        const hasEmptyContactName = this.contactList.some(
        ...
        const hasEmptyAddressContactName = this.addressList.some(
        ...
        const hasEmptyAddress = this.addressList.some(
    ...
    buildSubmitPayload() {
      ...
      payload.paymentTerm = this.resolvePaymentTermDictCode(
        payload.paymentTerm
      );
    ...
    resolvePaymentTermDictCode(value) {
      ...
      const match = list.find(
        (item) =>
          String(item.dictCode) === String(value) ||
    ...
    async loadCustomerContacts(customerId) {
      ...
      try {
        ...
        if (res.code === 200 && res.data) {
          ...
          // 记录原始联系人ID
          this.originalContactIds = this.contactList
            .map((c) => c.id)
            .filter((id) => id);
        } else {
          this.contactList = [];
    ...
    async loadCustomerAddresses(customerId) {
      ...
      try {
        ...
        if (res.code === 200 && res.data) {
          this.addressList = (res.data.list || res.data || []).map(
            (address) => ({
              id: address.id,
              contactName: address.contactName || "",
              contactPhone: address.contactPhone || "",
          ...
          // 记录原始收货地址ID
          this.originalAddressIds = this.addressList
            .map((a) => a.id)
            .filter((id) => id);
        } else {
          this.addressList = [];
