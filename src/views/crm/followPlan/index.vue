...
export default {
  ...
  methods: {
    ...
    updateCustomerSearchOptions() {
      const customerField = this.searchFields.find(
        (field) => field.key === "customerId"
      );
      if (customerField && customerField.props) {
        customerField.props.options = this.customerData.data.map(
          (customer) => ({
            label: customer.name,
            value: customer.id,
          })
      ...
      try {
        const params = {
          ...
          planState:
            this.searchForm.planState === ""
              ? undefined
              : Number(this.searchForm.planState),
    ...
    async handleDelete(row) {
      try {
        await this.$confirm(
          `确认删除计划"${row.planContent.substring(0, 20)}..."吗？`,
          "确认删除",
          { type: "warning" }
        );
    ...
    updateExecutorSearchOptions() {
      const executorField = this.searchFields.find(
        (field) => field.key === "planExecutor"
      );
...
<style scoped>
.crm-follow-plan-table :deep(.el-table__row:hover) {
