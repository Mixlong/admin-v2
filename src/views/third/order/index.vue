  ...
  <div class="app-container">
    <div class="toolbar">
      <transition name="fade-transform-tb">
        <el-form ref="queryForm" class="search-form" :model="queryParams" :inline="true" v-show="showSearch">
          ...
          <el-form-item label="Bom编号" prop="bomCode">
            <el-input v-model="queryParams.bomCode" filterable placeholder="请输入Bom编号" clearable />
          </el-form-item>
          <el-form-item label="所属品类" prop="categoryName">
            <el-select v-model="queryParams.categoryName" filterable allow-create clearable :loading="isCateLoading"
              loading-text placeholder="请选择所属品类" @change="changeCategory">
      ...
      <el-row>
        <el-col :span="1.5">
          <el-button type="primary" icon="el-icon-plus" @click="handleAdd" v-hasPermi="['third:order:add']">
            新增
          </el-button>
        </el-col>
      </el-row>
    ...
    <el-table v-loading="loading" :height="tableHeight(-30)" border :data="list">
      <el-table-column label="序号" width="58" type="index" align="center" fixed="left">
        <template slot-scope="scope">
          {{ (queryParams.p - 1) * queryParams.l + scope.$index + 1 }}
        </template>
      </el-table-column>
      <el-table-column label="客户名称" align="center" prop="customerName" width="120" fixed="left">
        <span slot-scope="{ row }" v-NoData="row.customerName"></span>
      </el-table-column>
      <el-table-column label="迪太订单号" align="center" prop="salesOrderNo" width="140" fixed="left" />
      <el-table-column label="客户订单号" align="center" prop="customerOrderNo" width="160" fixed="left">
      ...
      <el-table-column label="型号" align="center" prop="computerName" width="140">
        <span slot-scope="{ row }" v-NoData="row.computerName"></span>
      </el-table-column>
      <el-table-column label="BOM编码" align="center" prop="bomCode" width="130">
        <span slot-scope="{ row }" v-NoData="row.bomCode"></span>
      </el-table-column>
      <el-table-column label="芯片版本" align="center" prop="chipVersion" width="90">
      ...
      <el-table-column label="未发货数量" align="center" prop="unshippedNum" width="100">
      ...
      <el-table-column label="操作" align="center" width="210" fixed="right">
        <template slot-scope="{ row }">
          <div class="flex flex-start">
            ...
            <el-button type="text" v-hasPermi="['third:order:detail']" @click="seeDetail(row.id)">
              订单详情
            </el-button>
          </div>
          <div class="flex flex-start">
            <el-button class="text-red" type="text" v-hasPermi="['third:order:update']" @click="handleUpdate(row.id)">
              编辑
            </el-button>
            <!-- 审核 -->
            <el-button type="text" @click="onOrderAuth(row.id)" v-hasPermi="['third:order:check']"
              v-show="row.status === 0">
              审核
            </el-button>

            <el-button class="text-gray" type="text" @click="onOrderCancel(row.id)" v-hasPermi="['third:order:cancel']"
              v-show="row.status !== 2">
              取消
            </el-button>
            <el-button type="text" v-hasPermi="['third:order:log']" @click="onEditLog(row.id)">
              日志
            </el-button>

            <Tooltip v-hasPermi="['third:order:copy']" icon="el-icon-document-copy" content="复制"
              @click="rowDbClick(row)" />

            <Tooltip v-hasPermi="['third:order:planSchedule']" icon="el-icon-position" content="排产管理" @click="
            ...
            <Tooltip icon="el-icon-box" content="发货管理" @click="
    ...
    <orderDetail ref="orderDetailRef" :statusList="statusList" :productStatusList="productStatusList" />
