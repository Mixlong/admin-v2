<template>
	<!--   -->
	<el-dialog :close-on-click-modal="false" :title="title" :visible.sync="dialogVisible" :width="isDigWidth"
		append-to-body :top="boleConfig ? '5vh' : '15vh'">
		<el-form ref="form" :model="form" :rules="rules" label-width="110px" class="form-data form-data-inline"
			...
			<template v-if="!isBatchSync">
				...
				<el-form-item label="型号" prop="computerId">
					<el-select :disabled="form.id ? true : false" v-model="form.computerId" clearable size="small"
						@change="$forceUpdate()">
						<el-option v-for="dict in computerFormOptions" :key="dict.model" :label="dict.name"
							:value="dict.model" />
					</el-select>
				</el-form-item>
