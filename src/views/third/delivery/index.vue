<template>
	<div class="app-container">
		<transition name="fade-transform-tb">
			<el-form ref="queryForm" :model="queryParams" :inline="true" v-show="showSearch">
				<el-form-item label="客户名称" prop="customerName">
					<select-loadMore style="width: 100%" v-model="queryParams.customerName" :data="customerData.data"
				...
				<el-form-item label="迪太订单号" prop="salesOrderNo">
					<el-input v-model="queryParams.salesOrderNo" filterable placeholder="请输入迪太订单号" clearable />
				</el-form-item>
				<el-form-item label="客户订单号" prop="customerOrderNo">
					<el-input v-model="queryParams.customerOrderNo" filterable placeholder="请输入客户订单号" clearable />
				</el-form-item>
				<el-form-item label="所属品类" prop="categoryName">
					<el-select v-model="queryParams.categoryName" filterable allow-create clearable
						:loading="isCateLoading" loading-text placeholder="请选择所属品类" @change="changeCategory">
						<el-option v-for="dict in dictList" :key="dict.id" :label="dict.name" :value="dict.name" />
					</el-select>
				</el-form-item>
				<el-form-item label="仪表型号" prop="computerName">
					<el-select filterable remote clearable v-model="queryParams.computerName" placeholder="请选择仪表型号"
				...
				<el-form-item label="发货状态" prop="status">
					<el-select v-model="queryParams.status" placeholder="请选择发货状态" clearable @change="handleQuery">
						<el-option v-for="(label, value) in statusList" :key="value" :label="label" :value="value">
						</el-option>
					</el-select>
				</el-form-item>
				<el-form-item>
					<el-button type="primary" icon="el-icon-search" v-hasPermi="['third:delivery:query']"
						@click="handleQuery">
						搜索
					</el-button>
					<el-button icon="el-icon-refresh" v-hasPermi="['third:delivery:reset']" @click="resetQuery">
						重置
					</el-button>
				</el-form-item>
			</el-form>
		...
		<el-row :gutter="10" class="mb8">
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-plus" @click="handleAdd" v-hasPermi="['delivery:add:btn']">
					新增
				</el-button>
			</el-col>
		...
		<el-table class="table-scrollContainer" ref="tableRef" v-loading="loading" :height="tableHeight()" :data="list">
			<el-table-column label="序号" width="58" type="index" align="center" fixed>
			...
			<el-table-column label="迪太订单号" align="center" prop="salesOrderNo" width="120" fixed />
			<el-table-column label="客户订单号" align="center" prop="customerOrderNo" width="120" fixed>
			...
			<el-table-column label="型号" align="center" prop="computerName" width="120">
				<span slot-scope="{ row }" v-NoData="row.computerName"></span>
			</el-table-column>
			<el-table-column label="发货方式" align="center" prop="transportMode" width="100">
				<span slot-scope="{ row }" v-NoData="row.transportMode"></span>
			</el-table-column>
			<el-table-column label="发货单号" align="center" prop="orderNumber" width="100">
				<span slot-scope="{ row }" v-NoData="row.orderNumber"></span>
			</el-table-column>
			<el-table-column label="发货图片" align="center" prop="shippingPicture" width="100">
				<template slot-scope="{ row }">
					<preview-img width="60px" height="60px" :isDisBadge="false" :url="row.shippingPicture" />
				</template>
			</el-table-column>
			...
			<el-table-column label="发货数量" align="center" prop="shippingNumber" width="100">
			...
			<el-table-column label="发货人" align="center" prop="shippingName" width="100">
			...
			<el-table-column label="发货时间" align="center" prop="shippingTime" width="140">
				<template slot-scope="{ row }">
			...
			<el-table-column label="操作" align="center" width="130" fixed="right">
				<template slot-scope="{ row }">
					...
					<Tooltip v-hasPermi="['delivery:cancel:btn']" v-show="row.status !== 2 && row.status !== 4"
						icon="el-icon-close" content="取消" @click="handleCancel(row)" />

					<el-popconfirm title="确定要删除吗？" @confirm="handleDelete(row)" v-hasPermi="['delivery:delete:btn']">
						<Tooltip style="margin: 0 10px" slot="reference" icon="el-icon-delete" :className="['text-red']"
							content="删除" />
					</el-popconfirm>

					<Tooltip v-hasPermi="['delivery:detail:btn']" icon="el-icon-tickets" content="发货详情"
		...
		<edit-delivery ref="compUpdate" :isOrderFlag="isOrderFlag" v-bind="editParams"></edit-delivery>

		<orderDetail ref="orderDetailRef" :statusList="statusList" :tagType="tagType" />
