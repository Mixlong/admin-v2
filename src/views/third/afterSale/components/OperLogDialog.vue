    ...
    >
      ...
      <el-table-column type="expand">
        <template slot-scope="scope">
          <div v-if="scope.row.fieldChanges && scope.row.fieldChanges.length > 0" class="expand-table-container">
            <div class="expand-table-title">详细变更信息：</div>
            <el-table
              :data="scope.row.fieldChanges"
              border
              size="small"
              class="expand-changes-table"
            >
              ...
              <el-table-column label="原值" align="left" min-width="200">
                <template slot-scope="{ row }">
                  <div class="table-cell-content old-value-cell">{{ formatFieldValue(row.oldValue) }}</div>
                </template>
              </el-table-column>
              <el-table-column label="新值" align="left" min-width="200">
                <template slot-scope="{ row }">
...
export default {
  ...
  methods: {
    ...
    handleExpandChange(row, expandedRows) {
      this.expandedRows = expandedRows.map(r => r.operId);
    },
    // 获取变更类型标签颜色
