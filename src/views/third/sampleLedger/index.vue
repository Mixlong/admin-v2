  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" @submit.native.prevent>
      <el-form-item label="类型" prop="typeName">
        <el-select size="small" placeholder="请选择类型" clearable v-model="queryParams.typeName" @change="handleQuery">
          <el-option v-for="(item, index) in sampleTypeList" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="产品品类" prop="name">
        <el-select v-model="queryParams.name" placeholder="请选择产品品类" clearable filterable @change="getList">
          <el-option v-for="dict in dictList" :key="dict.id" :label="dict.name" :value="dict.name" />
        </el-select>
      </el-form-item>
      <el-form-item label="产品型号" prop="computerName">
        <el-select v-model="queryParams.computerName" placeholder="请选择产品型号" clearable filterable @change="getList">
          <el-option v-for="dict in modelList" :key="dict.id" :label="dict.name" :value="dict.name" />
        </el-select>
      </el-form-item>
      ...
      <el-row :gutter="10" class="mt5 fr">
        <el-col :span="1.5">
          <el-button type="primary" icon="el-icon-plus" size="mini" v-if="checkRole(['sale', 'admin'])"
            @click="handleAdd('compUpdate', '新增样品')">新增</el-button>
        </el-col>
      </el-row>
    </el-form>
    ...
    <el-table v-loading="loading" :data="list" :height="tableHeight()" border :cell-class-name="cellClassName"
      ...
      <el-table-column label="类型" prop="type" align="center" width="60" show-overflow-tooltip />
      <el-table-column label="产品品类" prop="name" align="center" width="80" show-overflow-tooltip />
      <el-table-column label="产品型号" prop="computerName" align="center" width="80" show-overflow-tooltip />
      <el-table-column label="ERP编码" prop="code" align="center" width="120" />
      <el-table-column label="配置信息" prop="code" align="left" header-align="center" width="200">
        <template slot-scope="{ row }">
          ...
          <p v-show="row.controllerHead">控制器接头：{{ row.controllerHead }}</p>
          <p v-show="row.controllerLine">控制器出线线长(不含头)：{{ row.controllerLine }}</p>
          <p v-show="row.keyCode">按键型号：{{ row.keyCode }}</p>
          <p v-show="row.keyLine">按键线长：{{ row.keyLine }}</p>
        </template>
      </el-table-column>
      <el-table-column label="生产时间" prop="productionTime" align="center" width="140" />
      <el-table-column label="生产来源" prop="productionSource" align="center" width="90" />
      <el-table-column label="对应客户" prop="customerName" align="center" width="80" />
    ...
    <pagination v-show="total > 0" :total="total" :page.sync="queryParams.p" :limit.sync="queryParams.l"
      @pagination="getList" />
    <CompUpdate ref="compUpdate" :sampleTypeList="sampleTypeList" :dictList="dictList" :modelList="modelList" />
