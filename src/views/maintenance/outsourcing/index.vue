  ...
  <div class="maintenance-record-container app-container">
    ...
    <IntelligentSearchForm 
      ...
      <template #page-actions>
        <el-button type="primary" @click="handleAdd" icon="el-icon-plus" size="mini">
          新增
        </el-button>
      </template>
    </IntelligentSearchForm>
      ...
      <el-table 
        ...
        <el-table-column label="操作" width="200" align="center" fixed="right">
          <template slot-scope="scope">
            ...
            <el-button size="mini" type="text" @click="handleCopy(scope.row)" icon="el-icon-document-copy">复制</el-button>
            <el-button size="mini" type="text" class="text-red" @click="handleDelete(scope.row)" icon="el-icon-delete">删除</el-button>
          </template>
        </el-table-column>  
      </el-table>
...
import { 
  getOutsourcingRecordList, 
  deleteMaintenanceRecord,
  exportOutsourcingRecord
} from '@/api/maintenance/record'
import { getOutsourcingCategoryList } from '@/api/third/fileConfig'
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      getOutsourcingRecordList(params).then((res) => {
    ...
    // 加载品类选项（外协机型，本地开发：647，正式服：825）
    loadCategoryOptions() {
      getOutsourcingCategoryList().then(res => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.$modal.confirm('是否确认导出当前搜索条件下的外协维修记录数据项?').then(() => {
        ...
        return exportOutsourcingRecord(params);
      }).then(response => {
