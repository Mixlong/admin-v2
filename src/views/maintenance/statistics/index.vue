...
export default {
  ...
  methods: {
    ...
    async processAndUpdateCharts() {
      console.log('开始查询图表数据...')
      console.log('产品类型图表参数:', this.chartParams.productType)

      try {
        const productTypeData = await this.fetchSingleChartData('productType')
...
.maintenance-statistics-container {
  ...
  height: calc(100vh - 84px); /* Subtract typical navbar height */
  display: flex;
...
.maintenance-statistics-container .chart-card-large {
  ...
  flex-direction: column;
  flex: 1;
  min-height: 600px; /* Increased base height */
}
