  ...
  <div class="config-overview">
    <el-form :model="queryParams" ref="queryForm" :inline="true">
      ...
      <el-form-item label="产品图纸状态" prop="specificationAuditStatus">
        <el-select
          v-model="queryParams.specificationAuditStatus"
          @change="getList"
          clearable
          style="width: 120px"
        >
      ...
      <el-button
        class="fr"
        type="primary"
        @click="addDialogChange"
        v-hasPermi="['config:overview:first:add']"
      >
        新增配置
      </el-button>
    ...
    <div style="position: relative">
      ...
      >
            ...
            <div class="audit-container">
              ...
              <div class="audit-row audit-row-config">
                <div class="audit-status">
                  <el-tag size="mini" v-if="row.state === 0" type="warning"
                    ><span class="tag-prefix-config">配置:</span>未审核</el-tag
                  >
                  <el-tag size="mini" v-else-if="row.state === 1" type="success"
                    ><span class="tag-prefix-config">配置:</span
                    >初审通过</el-tag
                  >
                  <el-tag size="mini" v-else-if="row.state === 2" type="danger"
                    ><span class="tag-prefix-config">配置:</span
                    >初审未通过</el-tag
                  >
                  <el-tag size="mini" v-else-if="row.state === 3" type="success"
                    ><span class="tag-prefix-config">配置:</span
                    >终审通过</el-tag
                  >
                  <el-tag size="mini" v-else-if="row.state === 4" type="danger"
                    ><span class="tag-prefix-config">配置:</span
                    >终审未通过</el-tag
                  >
                  ...
                  <!-- 配置审核人信息 - 移到状态后面 -->
                  <el-tooltip
                    v-if="row.createBy || row.firstPerson || row.lastPerson"
                    placement="top"
                  >
                    <div slot="content" style="line-height: 1.8">
                      <div v-if="row.createBy">创建者：{{ row.createBy }}</div>
                      <div v-if="row.firstPerson">
                        初审者：{{ row.firstPerson }}
                      </div>
                      <div v-if="row.lastPerson">
                        终审者：{{ row.lastPerson }}
                      </div>
                    </div>
                    <i
              ...
              <div v-if="row.packagingInfo" class="audit-row audit-row-package">
                <div class="audit-status">
                  <el-tooltip
                    v-if="row.packagingAuditStatus === -1"
                  ...
                  >
                    <el-tag size="mini" type="danger" style="cursor: pointer"
                      ><span class="tag-prefix-package">包装:</span>拒审</el-tag
                    >
                  </el-tooltip>
                  <el-tag
                    size="mini"
                    v-else-if="row.packagingAuditStatus === 0"
                    type="warning"
                    ><span class="tag-prefix-package">包装:</span>待审核</el-tag
                  >
                  <el-tag
                    size="mini"
                    v-else-if="row.packagingAuditStatus === 1"
                    type="success"
                    ><span class="tag-prefix-package">包装:</span>已审核</el-tag
                  >
                  <el-tag size="mini" v-else type="info"
                    ><span class="tag-prefix-package">包装:</span>--</el-tag
                  >

                  <!-- 包装审核人信息 - 移到状态后面 -->
                  <el-tooltip
                    v-if="row.packagingFirstPerson || row.packagingLastPerson"
                    placement="top"
                  >
                    <div slot="content" style="line-height: 1.8">
                      <div v-if="row.packagingLastPerson">
                        终审者：{{ row.packagingLastPerson }}
                      </div>
                    </div>
                    <i
                      class="el-icon-user"
                ...
                <div class="audit-operations">
                  <!-- 包装信息审核 -->
                  <Tooltip
                    v-if="row.packagingAuditStatus === 0"
                    class="text-orange"
                    icon="el-icon-circle-check"
                    content="包装审核"
                    v-hasPermi="['config:overview:packaging:final:check']"
                    @click="handlePackagingFinalAudit(row)"
                  />
                  ...
                  <Tooltip
                    icon="el-icon-edit"
                    content="包装编辑"
                    v-hasPermi="['config:overview:first:editPackage']"
                    @click="handleEditPackagingInfo(row)"
                  />
                </div>
              </div>
              ...
              <div class="audit-row audit-row-spec">
                <div class="audit-status">
                  ...
                  >
                    <el-tag size="mini" type="danger" style="cursor: pointer"
                      ><span class="tag-prefix-spec">图纸:</span>拒审</el-tag
                    >
                  </el-tooltip>
                  <el-tag
                    size="mini"
                  ...
                  <el-tooltip
                    v-if="
                      row.specificationFirstPerson ||
                      row.specificationLastPerson
                    "
                    placement="top"
                  >
                    <div slot="content" style="line-height: 1.8">
                      <div v-if="row.specificationFirstPerson">
                        初审者：{{ row.specificationFirstPerson }}
                      </div>
                      <div v-if="row.specificationLastPerson">
                        终审者：{{ row.specificationLastPerson }}
                      </div>
                    </div>
                    <i
                      class="el-icon-user"
                ...
                <div class="audit-operations">
                  ...
                  <Tooltip
                    ...
                    icon="el-icon-circle-check"
                    content="图纸初审"
                    v-hasPermi="['config:overview:specification:first:check']"
                    @click="handleSpecificationFirstAudit(row)"
                  />
                  ...
                  <Tooltip
                    ...
                    icon="el-icon-circle-check"
                    content="图纸终审"
                    v-hasPermi="['config:overview:specification:final:check']"
                    @click="handleSpecificationFinalAudit(row)"
                  />
        ...
        <el-table-column
          label="基础信息"
          align="left"
          label-class-name="group-header-basic"
        >
          <el-table-column
            label="型号类型"
            prop="isBist"
          ...
          >
            <template slot-scope="{ row }">
              ...
              >
                <el-tag
                  size="small"
                  :type="row.isBist == 1 ? 'success' : 'danger'"
                >
                  {{ row.isBist == 1 ? "BIST型号" : "大货型号" }}
                </el-tag>
                <div
                  style="font-size: 12px; color: #606266; text-align: center"
                >
                  {{ row.isBist == 1 ? "关联大货型号：" : "关联BIST型号：" }}
                  {{
                    (row.relatedModelList || [])
                      .map((item) => item.computerName || item.name || "-")
                      .join(", ") || "--"
                  }}
                </div>
              </div>
          ...
          <el-table-column
            ...
            align="center"
            min-width="160"
            class-name="rich-text-cell-column"
          >
            <template slot-scope="{ row }">
              <div
        ...
        <el-table-column
          label="外观信息"
          align="center"
          label-class-name="group-header-appearance"
        >
          <el-table-column
            label="产品图纸"
            prop="specification"
          ...
          >
            <template slot-scope="{ row }">
              <span class="text-green" v-if="row.controlHead === 1"
                >（含头）</span
              >
              <p>
          ...
          >
            <span
              slot-scope="{ row }"
              v-NoData="dicts_keyType_list[row.keyType]"
            />
          </el-table-column>
          <el-table-column
            label="按键连接类型"
          ...
          >
            <template slot-scope="{ row }">
              <span class="text-green" v-if="row.keyLineType === 1"
                >（含头）</span
              >
              <p>
        ...
        <el-table-column
          label="车型配置"
          align="center"
          label-class-name="group-header-config"
        >
          <el-table-column
            label="蓝牙"
            prop="bluetooth"
          ...
          >
            <span
              slot-scope="scope"
              v-NoData="dicts_unit[scope.row.unit]"
            ></span>
          </el-table-column>
          <el-table-column
            label="LOGO界面"
          ...
          >
            <span
              slot-scope="scope"
              v-NoData="dicts_logo[scope.row.logo]"
            ></span>
          </el-table-column>
          <el-table-column
            label="开机LOGO"
        ...
        >
          <el-table-column label="附件出货方式" align="center">
            ...
            <el-table-column label="按键螺丝" align="center" width="120">
              <template slot-scope="{ row }">
                <span
                  v-NoData="getPackagingValue(row, '解件出库方式', '按键螺丝')"
                ></span>
              </template>
            </el-table-column>
            <el-table-column label="硅胶垫片" align="center" width="120">
              <template slot-scope="{ row }">
                <span
                  v-NoData="getPackagingValue(row, '解件出库方式', '硅胶垫片')"
                ></span>
              </template>
            </el-table-column>
            <el-table-column
            ...
            >
              <template slot-scope="{ row }">
                <div
                  v-if="
                    !isRichTextEmpty(
                      getPackagingValue(row, '解件出库方式', '其它附件要求')
                    )
                  "
                  v-html="
                    getPackagingValue(row, '解件出库方式', '其它附件要求')
          ...
          >
            <template slot-scope="{ row }">
              <div>
                <div class="select-value-text">
                  {{ getPackagingValue(row, "附件装箱方式", "value") || "--" }}
                </div>
                <div
                  v-if="
                    ...
                    !isRichTextEmpty(
                      getPackagingValue(row, '附件装箱方式', 'administrator')
                    )
                  "
                  v-html="
                    getPackagingValue(row, '附件装箱方式', 'administrator')
          ...
          >
            <template slot-scope="{ row }">
              <div>
                <div class="select-value-text">
                  {{ getPackagingValue(row, "箱唛要求", "value") || "--" }}
                </div>
                <div
                  v-if="
                    shouldShowAdministrator(
                    ...
                    !isRichTextEmpty(
                      getPackagingValue(row, '箱唛要求', 'administrator')
                    )
                  "
          ...
          >
            <template slot-scope="{ row }">
              <div>
                <div class="select-value-text">
                  {{ getPackagingValue(row, "检验报告要求", "value") || "--" }}
                </div>
                <div
                  v-if="
                    shouldShowAdministrator(
                    ...
                    !isRichTextEmpty(
                      getPackagingValue(row, '检验报告要求', 'administrator')
                    )
                  "
                  v-html="
                    getPackagingValue(row, '检验报告要求', 'administrator')
...
import {
  ...
  packagingFirstAudit,
  packagingFinalAudit,
  specificationFirstAudit,
  specificationFinalAudit,
...
export default {
  ...
  methods: {
    ...
    initTableDrag() {
      ...
      const scrollContainer = this.$refs.tableRef?.$el?.querySelector(
    ...
    async handlePackagingInfoSave(newFormatData) {
      ...
      try {
        ...
        const detailRes = await this.getComputerDetail(
          this.currentEditRow.computerId
        );
        ...
        if (res.code === 200) {
          ...
          console.log(
            "✅ 数据更新完成，packagingInfo:",
            this.currentEditRow.packagingInfo
          );
    ...
    async handleAuditConfirm() {
      ...
      if (this.auditDialog.form.status === -1) {
        if (
          !this.auditDialog.form.why ||
          this.auditDialog.form.why.trim() === ""
        ) {
    ...
    handleExport() {
      if (!this.uploadIds.length)
        return this.msgWarning("请选择导出的数据项！");
      ...
      try {
        ...
        if (convertRes.data.code === 200 && convertRes.data.data) {
          ...
          if (Array.isArray(convertRes.data.data)) {
            ...
            const imageUrls = convertRes.data.data
              .map((item) => {
                const url = item?.url || item;
                // 清理URL中的反引号和空格
                return typeof url === "string"
