<template>
  <div class="app-container">
    ...
    @search="handleSearch"
        ...
        <!-- 自定义品类字段 -->
        <template #field-inventoryInfoId="{ field, searchForm }">
        <el-form-item label="来料记录" prop="inventoryInfoId">
                <select-loadMore
                  v-model="searchForm.inventoryInfoId"
                  style="width: 100%"
                  ...
                  placeholder="请选择来料记录"
                  size="mini"
                  @change="handleAutoSearchForInventory($event)"
                 > 
                </select-loadMore>
        ...
        <template #page-actions>
            <el-button type="primary" @click="handleAdd" icon="el-icon-plus" size="mini" v-hasPermi="['defect-processing:add']"
             >
                新增
            </el-button>
            <el-button type="info" plain @click="handleAddPeople" icon="el-icon-user" v-hasPermi="['defect-processing:people']"
                size="mini">
                人员管理
            </el-button>
        </template>
    ...
    <el-table v-loading="loading" :data="bomChangeList" :height="tableHeight()" border>
      ...
      <el-table-column label="MRB编号"  prop="ecn"   align="center">
      </el-table-column>
      <el-table-column label="来料记录"  prop="inventoryInfoId"   align="center">
        <template slot-scope="scope">
          <el-link @click='openDetail(scope.row)' :underline="true" type="primary">{{scope.row.inventoryInfoId}}</el-link>
        </template>
      </el-table-column>
      
      <el-table-column label="来料编码" prop="invCode" align="center"   width="110">
        <template slot-scope="scope">
          <span>{{ scope.row.invCode || '--' }}</span>
        </template>
      </el-table-column>
      <el-table-column label="物料名称" prop="invName" align="center" width="150" show-overflow-tooltip>
        <template slot-scope="scope">
      ...
      <el-table-column label="检测数据" prop="englishName" align="center" width="140">
        <template slot-scope="scope">
            <div class="info-item">
              <label>到料数量：</label>
      ...
      <el-table-column label="会审状态" prop="list" align="center" width="350">
      ...
      <el-table-column label="确认人" align="center" width="80">
        <template slot-scope="{ row }">
          <div v-if="row.handlerPerson">
            <div>{{ row.handlerPerson }}</div>
            <el-tag v-if="row.handlerState === 1" type="success" size="mini">已通过</el-tag>
            <el-tag v-else-if="row.handlerState === 2" type="danger" size="mini">已驳回</el-tag>
            <el-tag v-else type="info" size="mini">待处理</el-tag>
          </div>
          <span v-else>-</span>
        </template>
      </el-table-column>
      <el-table-column label="创建时间" prop="createTime" align="center" width="97" />
      <el-table-column label="操作" align="center" fixed="right" width="120">
        <template slot-scope="{ row }">
          <el-button size="mini" type="primary" icon="el-icon-view" @click="handleView(row)">查看</el-button>

          <el-button v-if="row.firstState !== 1 && !isAllReviewsPassed(row)" size="mini" type="primary" icon="el-icon-edit"
          ...
          <template v-for="(item, index) in (row && row.list ? row.list : [])">
            <!-- 简化的会审按钮 - 只要状态为0且是当前用户就显示 -->
            <el-tooltip
              v-if="item.state === 0 && item.fieldName === nickName"
              :content="`${TriageList[item.field] || '部门'}会审`" placement="top" :key="'review-' + index">
              <el-button size="mini" type="primary" @click="handleAuthFlag(item, 2, item.field)">
                <i class="el-icon-check"></i> {{ TriageList[item.field] || '部门' }}会审
              </el-button>
            </el-tooltip>

            <!-- 简化的撤回会审按钮 - 只要状态为1且是当前用户就显示，但所有会审完成后或确认人完成后不显示 -->
            <el-tooltip
              v-if="item.state === 1 && item.fieldName === nickName && !isAllReviewsPassed(row) && !(row.handlerPerson && row.handlerState !== 0)"
              :content="`撤销${TriageList[item.field] || '部门'}会审`" placement="top" :key="'reset-' + index">
              <el-button size="mini" type="danger" @click="handleResetCheck(item, 2, item.field)">
                <i class="el-icon-refresh-left"></i> 撤回{{ TriageList[item.field] || '部门' }}会审
              </el-button>
            </el-tooltip>
          </template>

          <!-- 确认人审核按钮 - 会审完成后显示 -->
          <el-button v-if="row && row.handlerPerson === nickName && isAllReviewsPassed(row) && row.handlerState === 0" 
            size="mini" type="success" @click="handleHandlerReview(row)" style="margin-left: 5px;">
            <i class="el-icon-s-check"></i> 处理审核
          </el-button>

          <!-- 审核流程按钮 - 直接显示 -->
          <!-- 简化的初审按钮 - 只要是初审人且状态为0就显示 -->
          <el-button v-if="row && row.firstPerson === nickName && row.firstState === 0"
    ...
    <!-- 添加或修改BOM变更对话框 -->
    <AddDefectProcessingDialog ref="bomChangeForm" @success="getList" />
    <!-- 查看详情对话框 -->
      ...
      <el-row type="flex" justify="center">
        <el-col :xs="24" :span="20">
          <el-form ref="peopleForm" :model="peopleManageForm" :rules="peopleManageRules" label-width="100px"
            ...
            <el-form-item label="涉及领域："></el-form-item>
    ...
    <el-dialog title="确认人审核" :visible.sync="handlerReviewVisible" width="600px" append-to-body :close-on-click-modal="false">
      <el-form :model="handlerForm" ref="handlerForm" label-width="100px">
        ...
        <el-form-item label="确认意见" prop="remark" :rules="[{required: true, message: '请输入确认意见'}]">
          <el-input type="textarea" v-model="handlerForm.remark" :rows="3" placeholder="请输入确认意见"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="handlerReviewVisible = false">取消</el-button>
        <el-button type="primary" @click="submitHandlerReview">确定</el-button>
      </div>
    </el-dialog>
...
<script>
import AddDefectProcessingDialog from './components/AddDefectProcessingDialog'
...
import ViewInspectionDataDialog from '../inspection-data/components/ViewInspectionDataDialog'
import {
...
import {getIqcDefectProcessingList, editDefectHandlerPersonnel,deleteIqcDefectProcessing,getDefectHandlerPersonnelList,getDefectHandlerNoticeInfo,defectHandlerNoticeFieldState,defectHandlerNoticeHandlerState} from '@/api/iqc/defectProcessing.js'
import { personBomEdit } from "@/api/third/ecn"
import {getDetailInfo } from "@/api/iqc/inspectionData"
...
export default {
  ...
  data() {
    return {
      ...
      authFormRules: {
        ...
        orderChangeStatus: [{ required: true, message: '请选择订单是否已做变更', trigger: 'change' }],
  ...
  methods: {
    ...
    handleSearch() {
      ...
      this.queryParams.ecn = this.searchForm.ecn
      this.queryParams.inventoryInfoId = this.searchForm.inventoryInfoId
      this.queryParams.category = this.searchForm.category
    ...
    openDetail(row){
      ...
      getDetailInfo(row.inventoryInfoId).then(res => {
    ...
    handleReset() {
      ...
      // 重置查询参数
      this.queryParams.ecn = null
      this.queryParams.inventoryInfoId = null
      this.queryParams.category = null
      this.queryParams.invCode = null
      this.queryParams.invName = null
    ...
    // 处理自定义字段的自动搜索
    handleAutoSearchForCustomField(field, value) {
      // 更新字段值
    ...
    // 处理来料记录字段的自动搜索
    handleAutoSearchForInventory(value) {
      // 自动触发搜索
      this.$nextTick(() => {
        this.handleSearch();
      });
    ...
    checkUrlParams() {
      ...
      // 检查是否有inspectionDataId参数
      if (params.inspectionDataId) {
        console.log('检测到inspectionDataId参数:', params.inspectionDataId)
        // 直接弹出新增对话框，并设置来料记录ID
        this.$nextTick(() => {
          this.handleAdd(params.inspectionDataId)
        })
        // 删除URL中的inspectionDataId参数
        const newQuery = { ...this.$route.query }
        delete newQuery.inspectionDataId
        this.$router.replace({
    ...
    getReviewTooltip(row) {
      ...
      // 按部门分组显示会审人员
      const grouped = this.getReviewStatusByField(row.list);
      const details = [];

      Object.keys(grouped).forEach(field => {
        const departmentName = this.TriageList[field];
        const departmentItems = row.list.filter(item => item.field === field);
      ...
      return `会审状态: ${passedReviews}/${totalReviews} (${reviewStatus})\n\n${details.join('\n\n')}`;
    },
    ...
    getSystemStepClass(row) {
      if (!row.systemPerson || row.secondState !== 1) return 'step-disabled';
    ...
    getSystemStepIcon(row) {
      if (!row.systemPerson || row.secondState !== 1) return 'el-icon-setting';
    ...
    getOrderStepClass(row) {
      ...
      // 检查前置条件：系统变更通过，且当前可以进行订单变更
      if (row.systemState === 1 && row.orderChangeState === 0) return 'step-active';
      return 'step-pending';
    },
    ...
    getOrderStepIcon(row) {
      ...
      // 检查前置条件：系统变更通过，且当前可以进行订单变更
      if (row.systemState === 1 && row.orderChangeState === 0) return 'el-icon-loading';
    ...
    getList() {
      this.loading = true;
      getIqcDefectProcessingList(this.queryParams).then(response => {
    ...
    /** 新增按钮操作 */
    handleAdd(inventoryInfoId = null) {
      this.$refs.bomChangeForm.openDialog(null, inventoryInfoId);
    },
    ...
    async handleUpdate(row) {
      ...
      // 请求详情
      const res = await getDefectHandlerNoticeInfo(id)
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm(`是否确认${action}该订单BOM变更通知？`).then(() => {
        ...
        return deleteIqcDefectProcessing(data);
      }).then(() => {
        this.getList();
    ...
    getPeopleList(type) {
      return new Promise((resolve) => {
        try {
          getDefectHandlerPersonnelList({ type, p: 1, l: 50 }).then((res) => {
            const { list } = res.data;
            // 对人员数据进行去重处理
    ...
    submitPeopleForm() {
      this.$refs["peopleForm"].validate((valid) => {
        if (valid) {
          ...
          editDefectHandlerPersonnel(list).then((res) => {
            if (res.data) {
    ...
    handleAuthFlag(row, isAuthFlag, field = null) {
      ...
      if (isAuthFlag === 7) {
        this.authForm = {
          ...
          orderChangeStatus: row.orderChangeStatus || '',
      ...
      this.$confirm("确定要撤销审核吗?", "提示", {
        ...
        .then(() => {
          ...
          if (flag === 2) {
            ...
            defectHandlerNoticeFieldState(data).then((res) => {
              if (res.data) {
                this.msgSuccess("操作成功");
                this.getList();
                this.authDialogVisible = false;
      ...
      this.$refs["authForm"].validate((valid) => {
        if (valid) {
          ...
          const convertedForm = {
            ...
            // 处理订单变更和工单变更的状态值
            orderChangeStatus: this.authForm.orderChangeStatus || '',
    ...
    // 确认人审核
    handleHandlerReview(row) {
      this.handlerForm.id = row.id;
      this.handlerForm.result = 1;
      this.handlerForm.remark = '';
      this.handlerForm.state = 0;
      this.handlerReviewVisible = true;
    },
    ...
    submitHandlerReview() {
      this.$refs["handlerForm"].validate((valid) => {
        if (valid) {
          const data = {
            id: this.handlerForm.id,
            result: this.handlerForm.result,
            remark: this.handlerForm.remark,
            state: this.handlerForm.result === 1 ? 1 : 2  // 1=通过, 2=驳回
          };

          defectHandlerNoticeHandlerState(data).then(response => {
            if (response.code === 200) {
              this.$modal.msgSuccess("处理成功");
              this.handlerReviewVisible = false;
              this.getList();
            } else {
              this.$modal.msgError(response.msg || "处理失败");
            }
          }).catch(error => {
            console.error("确认人审核失败:", error);
            this.$modal.msgError("处理失败，请检查网络连接");
          });
        }
      });
    },
