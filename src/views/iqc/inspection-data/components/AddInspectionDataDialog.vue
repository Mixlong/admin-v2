    ...
    <div class="dialog-content">
      <el-form 
        ...
        <fieldset class="form-fieldset">
          ...
          <el-row :gutter="20">
            <el-col :span="24">
              <el-form-item label="检验结果描述" prop="testInfo" label-width="120px">
                <Editor 
                    ...
                    setup: (editor) => {
                      editor.on('init', () => {
                        setTimeout(() => {
                          editor.getBody().blur()
                          // 确保父容器滚动位置不变
                          const dialogBody = document.querySelector('.inspection-data-dialog .el-dialog__body')
...
export default {
      ...
      if (val) {
        ...
        this.$nextTick(() => {
          const resetScrollImmediately = () => {
            const dialogBody = document.querySelector('.inspection-data-dialog .el-dialog__body')
            const dialogContent = document.querySelector('.inspection-data-dialog .dialog-content')
            if (dialogBody) dialogBody.scrollTop = 0
  ...
  methods: {
    ...
    fetchInventoryData() {
      ...
      if (this.inventorySearchForm.invCode) params.invCode = this.inventorySearchForm.invCode
        ...
        afterCategoryList({
          ...
          if (res.code === 200 && res.data) {
            ...
            if (this.editData && this.editData.category && !more) {
              ...
              const existsInList = this.categoryData.data.some(item => item.id === currentCategory)
    ...
    handleInventoryChange(invCode) {
      if (invCode) {
        const selectedInventory = this.inventoryData.data.find(item => item.invCode === invCode)
