<template>
  <div class="navbar-wrap-style">
    <div
      style="font-size: 25px; padding: 0 25px 0 25px"
      class="hamburger-container text-shadow text-blue"
    >
      <!-- 迪太云后台管理 -->
      <img
        :src="
      ...
      >
        <el-popover placement="top" width="620">
          <el-row :gutter="15">
            <el-col
              :span="6"
              v-for="(item, index) in themeImageCount"
              :key="index"
            >
              <div :class="{ 'select-theme': customImage == index }">
                ...
                <el-image
                  ...
                  class="pointer"
                  style="height: 77px"
                  :src="require('@/assets/theme/smallDemo' + index + '.png')"
                  @click="selectTheme(index)"
                ></el-image>
