  ...
  methods: {
    init() {
      ...
      this.$nextTick(() => {
        ...
        if (
          toolbarContainer &&
          toolbarContainer.classList.contains("ql-toolbar")
        ) {
          // 创建文件上传按钮
          ...
          fileButton.addEventListener("click", () => {
            ...
            if (this.$refs.fileUpload) {
              // 触发隐藏的文件上传输入框
              const input =
                this.$refs.fileUpload.$el.querySelector('input[type="file"]');
              if (input) {
                input.click();
              } else {
          ...
          hrButton.innerHTML =
            '<div style="width: 100%; height: 2px; background-color: #444; margin-top: 6px;"></div>'; // 简单的分割线图标样式
