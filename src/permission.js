...
router.beforeEach((to, from, next) => {
  ...
  console.log('[Permission] 路由守卫检查 - to:', to.path, '- from:', from.path, '- hasToken:', !!token, '- isWujie:', isWujie, '- roles:', store.getters.roles.length, '- retryCount:', retryCount);
  
  // 在微前端环境下，如果要跳转到 404
  if (isWujie && to.path === '/404') {
    // 检查重试次数，防止无限循环
    ...
    if (originalPath && originalPath !== '/404' && originalPath !== '/' && originalPath !== to.path) {
      // 如果 roles 为空，说明动态路由还没加载，记录原始路径等待加载完成
      if (store.getters.roles.length === 0) {
        pendingRoute = originalPath
        console.log('[Permission] 微前端环境，roles 为空，记录待处理路径:', pendingRoute)
        // 继续执行，让后面的逻辑去加载用户信息和动态路由
      } else if (!isLoadingRoutes) {
        // roles 已有值但还是跳转到 404，说明动态路由可能刚加载完，尝试重新导航
        retryCount++
        console.log('[Permission] 微前端环境，roles 已有值但跳转到 404，尝试导航到:', originalPath, '重试次数:', retryCount)
      ...
      if (store.getters.roles.length === 0) {
        ...
        store
          ...
          .then((res) => {
            ...
            const hasValidMenuData = isWujie && menuData && Array.isArray(menuData) && menuData.length > 0;

            if (hasValidMenuData) {
              try {
                console.log('[Permission] 微前端环境，使用 admin-v3 传来的菜单数据, 数量:', menuData.length);
                ...
                console.error('[Permission] 处理 menuData 失败，回退到独立模式:', err);
                // 继续执行下面的 GenerateRoutes
              }
            } else {
              console.log('[Permission] 微前端环境但 menuData 无效，回退到独立模式');
            }
            ...
            store.dispatch("GenerateRoutes", { roles }).then((accessRoutes) => {
              ...
              if (isWujie && pendingRoute) {
                const targetPath = pendingRoute
                pendingRoute = null
                console.log('[Permission] 微前端环境，导航到待处理路径:', targetPath)
              ...
              if (isWujie && to.path === '/404') {
                ...
                if (originalPath && originalPath !== '/404' && originalPath !== '/') {
                  console.log('[Permission] 微前端环境，从 404 导航到原始路径:', originalPath)
                  next({ path: originalPath, replace: true });
                  return;
                }
              }
